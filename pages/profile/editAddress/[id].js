...
import { setHeaderAuth, ValidateInput } from "utils";
...
const EditAddress = ({ id, addresses = [] }) => {
    ...
    useEffect(() => {
        if (thAddress.length > 0) {
            getAddressProfile();
        }
    }, [thAddress]);

    const getAddressProfile = async () => {
        const result = await ApiUsers.getUserAddressById(id);
        if (result.status == 200) {
            setState({
                ...result.data,
                sender_code: `${result.data.zipcode}_${result.data.subdistrict}_${result.data.district}_${result.data.province}`
            });
        }
    }
    ...
    const handleValidation = () => {
        ...
        if (!fields["phone"]) {
            ...
            errors["phone"] = ValidateInput(fields["phone"], "mobile");
    ...
    const submit = async (e) => {
        ...
        if (isValid) {
            try {
                const result = await ApiUsers.editAddress({
                    id: state?.id,
                    name: state.name,
                    phone: state.phone,
    ...
    return <div>
        ...
        <div className="ml-5 lg:ml-24">
            <TitleMenu title={"แก้ไขที่อยู่"} imageSrc="/assets/images/registerTitle.webp" />
        </div>
